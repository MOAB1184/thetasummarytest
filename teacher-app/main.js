...
ipcMain.handle('upload-audio', async (event, params) => {
    ...
    try {
        ...
        // Save recording metadata to S3 (optional, or keep for now)
        const recordingKey = `${store.get('currentSchool')}/teachers/${params.teacherEmail}/classes/${params.classCode}/recordings/${recordingId}.json`;
        ...
        const filePath = `${store.get('currentSchool')}/teachers/${params.teacherEmail}/classes/${params.classCode}/recordings/${recordingId}.${params.fileType}`;

        // POST to backend
        const backendUrl = process.env.BACKEND_URL || 'https://thetasummary.com/api';
        const response = await fetch(`${backendUrl}/api/wasabi/upload`, {
            method: 'POST',
        ...
        if (!response.ok) {
            return { success: false, error: data.error || 'Backend upload failed' };
        }
        console.log('Upload successful via backend:', filePath);
        return { success: true, recording: newRecording };
    } catch (error) {
        console.error('\n=== Audio Upload Error ===');
        console.error('Error type:', error.constructor.name);
...
ipcMain.handle('save-summary', async (event, { teacherEmail, classCode, summaryData }) => {
    try {
        console.log('[Main] Starting save-summary handler...');
        console.log('[Main] Parameters:', {
            teacherEmail,
            classCode,
            summaryName: summaryData.name,
            hasId: !!summaryData.id,
            type: summaryData.type
        });

        // Use provided id if present (for overwriting), otherwise generate new
        let summaryId;
        if (summaryData.id) {
            // If it's a full path, extract just the filename (no extension)
            const idPart = summaryData.id.includes('/') ? summaryData.id.split('/').pop().replace('.pdf', '').replace('.json', '') : summaryData.id;
            summaryId = idPart;
            console.log('[Main] Using existing ID:', summaryId);
        } else {
            summaryId = Date.now().toString();
            console.log('[Main] Generated new ID:', summaryId);
        }

        const isPDF = summaryData.name.toLowerCase().endsWith('.pdf');
        console.log('[Main] Is PDF:', isPDF);
        ...
        if (isPDF) {
            ...
            console.log('[Main] Saving PDF to key:', pdfKey);
            
            const pdfBuffer = Buffer.from(summaryData.content.split(',')[1], 'base64');
            console.log('[Main] PDF buffer size:', pdfBuffer.length);
            ...
            try {
                ...
                console.log('[Main] PDF successfully uploaded to S3');
            } catch (s3Error) {
                console.error('[Main] S3 upload error:', s3Error);
                throw s3Error;
            }
            ...
            console.log('[Main] Saving JSON summary to key:', summaryKey);
            ...
            try {
                ...
                console.log('[Main] JSON summary successfully uploaded to S3');
            } catch (s3Error) {
                console.error('[Main] S3 upload error:', s3Error);
                throw s3Error;
            }
        ...
        console.error('[Main] Error in save-summary:', error);
        return { success: false, error: error.message };
    }
});
...
ipcMain.handle('approve-summary', async (event, { teacherEmail, classCode, summaryId }) => {
  try {
    ...
    try {
      ...
      if (error.code === 'NoSuchKey') {
        // Try PDF path
        const pdfPath = `${school}/teachers/${teacherEmail}/classes/${classCode}/summaries/${summaryId}.pdf`;
        try {
          const pdfObj = await s3.getObject({
            Bucket: BUCKET_NAME,
            Key: pdfPath
          }).promise();
          ...
          // Copy PDF to approved location
          const approvedPdfPath = `${school}/teachers/${teacherEmail}/classes/${classCode}/approved-summaries/${summaryId}.pdf`;
          await s3.copyObject({
            Bucket: BUCKET_NAME,
            CopySource: `${BUCKET_NAME}/${pdfPath}`,
            Key: approvedPdfPath,
            ACL: 'public-read',
            CacheControl: 'public, max-age=31536000'
          }).promise();
          ...
          if (pdfError.code === 'NoSuchKey') {
            return { success: false, error: 'Summary not found or already approved/denied' };
          }
          throw pdfError;
        }
      }
    ...
    // Copy summary to approved location
    await s3.copyObject({
      Bucket: BUCKET_NAME,
      CopySource: `${BUCKET_NAME}/${summaryPath}`,
      Key: approvedPath,
      ACL: 'public-read',
      CacheControl: 'public, max-age=31536000'
    }).promise();
...
ipcMain.handle('deny-summary', async (event, { teacherEmail, classCode, summaryId }) => {
  try {
    // Delete both JSON and PDF summary files so it is never shown
    const school = store.get('currentSchool');
    const summaryJsonPath = `${school}/teachers/${teacherEmail}/classes/${classCode}/summaries/${summaryId}.json`;
    const summaryPdfPath = `${school}/teachers/${teacherEmail}/classes/${classCode}/summaries/${summaryId}.pdf`;
    let deleted = false;
    try {
      await s3.deleteObject({ Bucket: BUCKET_NAME, Key: summaryJsonPath }).promise();
      deleted = true;
    } catch (err) {
      // Ignore if not found
    }
    try {
      await s3.deleteObject({ Bucket: BUCKET_NAME, Key: summaryPdfPath }).promise();
      deleted = true;
    } catch (err) {
      // Ignore if not found
    }
...
ipcMain.handle('get-summaries', async (event, { teacherEmail, classCode, approved }) => {
    try {
        console.log('Getting summaries for teacher:', teacherEmail, 'class:', classCode, 'approved:', approved);
        const school = store.get('currentSchool');
        let prefix;
        if (approved) {
            prefix = `${school}/teachers/${teacherEmail}/classes/${classCode}/approved-summaries/`;
        } else {
            prefix = `${school}/teachers/${teacherEmail}/classes/${classCode}/summaries/`;
        }
        const params = {
            Bucket: BUCKET_NAME,
            Prefix: prefix
...
ipcMain.handle('open-pdf-editor', async (event, { pdfId, classCode }) => {
  const teacherEmail = store.get('userEmail');
  ...
  // Load the PDF editor HTML and pass the pdfId, classCode, and teacherEmail as query strings
  editorWindow.loadFile('pdf-editor.html', { query: { pdfId, classCode, teacherEmail } });
  if (process.env.NODE_ENV === 'development') {
